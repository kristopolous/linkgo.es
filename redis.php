...
class Redis {
    ...
    function &zadd($name, $value) {
        $this->connect();
        $this->_write("ZADD $name 1 " . strlen($value) . "\r\n$value\r\n");
        return $this->get_response();
    }
    function &zincrby($name, $value) {
        $this->connect();
        $this->_write("ZINCRBY $name 1 " . strlen($value) . "\r\n$value\r\n");
        return $this->get_response();
    }

    function &zrange($name, $start, $end) {
        $this->connect();
        $this->_write("ZRANGE $name $start $end\r\n");
        return $this->get_response();
    }
